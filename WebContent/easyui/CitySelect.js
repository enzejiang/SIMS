var s = [ "s1", "s2", "s3" ];
var add_s = [ "s1_add", "s2_add", "s3_add" ];
var edit_s = [ "s1_edit", "s2_edit", "s3_edit" ];
...
function change(v, fromName) {
	var str = "0";
	for (i = 0; i < v; i++) {
		str += ("_" + (document.getElementById(fromName[i]).selectedIndex - 1));
	}
 
	var ss = document.getElementById(fromName[v]);
	with (ss) {
		length = 0;
		options[0] = new Option(opt0[v], opt0[v]);
		if (v && document.getElementById(fromName[v - 1]).selectedIndex > 0 || !v) {
			if (dsy.Exists(str)) {
				ar = dsy.Items[str];
...
function CityChoose() {
	for (i = 0; i < s.length - 1; i++)
		document.getElementById(s[i]).onchange = new Function("change("+ (i + 1) + ",s);promptinfo();");
	change(0,s);
}

function Add_CityChoose() {
	for (i = 0; i < add_s.length - 1; i++)
		document.getElementById(add_s[i]).onchange = new Function("change("+ (i + 1) + ", add_s);promptinfo();");
	change(0,add_s);
}
function Edit_CityChoose() {
	for (i = 0; i < edit_s.length - 1; i++)
		document.getElementById(edit_s[i]).onchange = new Function("change("+ (i + 1) + ", edit_s);promptinfo();");
	change(0,edit_s);
}

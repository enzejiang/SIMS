	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				/* {field:'chk',checkbox: true,width:50}, */
 		        ...
 		        {field:'grade',title:'所属年级',width:100, formatter: function(value, row, index) {
 						if (row.grade) {
 				...
 				{field:'course',title:'班级课程',width:100, formatter: function(value, row, index) {
						if (row.course) {
				...
				{field:'teacher',title:'带班教师',width:100, formatter: function(value, row, index) {
					if (row.teacher) {
				...
				{field:'startTime',title:'开课时间',width:200},
				{field:'endTime',title:'结课时间',width:200},
	 		]], 
	        toolbar: "#toolbar"
	    }); 
	    //设置分页控件 
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectRow == null){
            	...
            	var classesId = selectRow.id;
            	$.messager.confirm("消息提醒", "将删除与班级相关的所有数据(包括学生)，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "ClassesServlet?method=delete",
	  	...
	  	$("#gradeList").combobox({
	  		...
	  		editable: false, //不可编辑
	  		method: "post",
	  		url: "GradeServlet?method=getGrid&t="+new Date().getTime(),
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "ClassesServlet?method=insert",
	...
	<div id="addDialog" style="padding: 10px">  
    	<form id="addForm" method="post">
	    	<table cellpadding="8" >
	    		...
	    		<tr>
	    			<td>带班教师：</td>
	    			<td><select id="add_teacherList" style="width: 200px; height: 30px;" name="teacherId" ></select></td>
	    		</tr>
	    		...
	    		<tr>
	    			<td>开课时间:</td>
	    			<td><input id= "add_start_time" style="width: 200px; height: 30px;" class="easyui-datebox" name="startTime" /></td>
	    		</tr>
	    		
	    		<tr>
	    			<td>结课时间:</td>
	    			<td><input id="add_end_time" style="width: 200px; height: 30px;" class="easyui-datebox" name="endTime" /></td>
	    		</tr>

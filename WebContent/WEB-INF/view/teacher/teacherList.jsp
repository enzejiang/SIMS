...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"TeacherServlet?method=getGrid&t="+new Date().getTime(),
	        idField:'id', 
	        ...
	        columns: [[  
				/* {field:'chk',checkbox: true,width:50}, */
 		        ...
 		        {field:'code',title:'教工号',width:150, sortable: true},    
 		        {field:'name',title:'姓名',width:150},
 		        {field:'gender',title:'性别',width:100},
 		        {field:'phone',title:'电话',width:150},
 		       	{field:'wechat',title:'微信号',width:100},
		        /* {field:'photo',title:'照片地址',width:150}, */
		        {field:'birthDay',title:'生日',width:100},
 		        {field:'entryDay',title:'入职时间',width:150},
 		       	{field:'basicSalary',title:'基本工资',width:100, formatter: function(value, row, index) {
					if (value) {
 				...
 				{field:'classfees',title:'课时费用',width:100, formatter: function(value, row, index) {
					if (value) {
						return value.toFixed(1);
					} else {
						return value;
 		        ...
 		        {field:'courseList',title:'主教课程',width:500, formatter: function(value, row, index) {
					if (row.course) {
        	...
        	if (selectLength == 0) {
            	...
            	$.messager.confirm("消息提醒", "将删除与教师相关的所有数据，确认继续？", function(r) {
            		if(r){
            			$.ajax({
							type: "post",
							url: "TeacherServlet?method=delete",
				...
				}, */
	    		{
					...
					handler:function(){
						...
						if (!validate) {
							...
							$.ajax({
								type: "post",
								url: "TeacherServlet?method=insert",
								data: $("#addForm").serialize(),
								success: function(data) {
									//var rsp = JSON.stringify(data);
	  ...
	  //下拉框通用属性
	  	$("#add_gradeList, #add_clazzList, #add_courseList, #majorCourseId").combobox({
	  		width: "200",
	  		height: "30",
	  		valueField: "id",
	  		textField: "name",
	  	...
	  	$("#majorCourseId").combobox({
	  		url: "CourseServlet?method=getGrid&t="+new Date().getTime(),
	  		onLoadSuccess: function(){
		  		//默认选择第一条数据
				var data = $(this).combobox("getData");;
				$(this).combobox("setValue", data[0].id);
	...
	<div id="addDialog" style="padding: 10px;">  
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			<td style="width:60px">教工号:</td>
	    			<td colspan="4">
	    				<input id="add_number"  class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="code" data-options="required:true, missingMessage:'请输入工号'" />
	    			</td>
	    			
	    			<td>姓名:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="name" data-options="required:true, missingMessage:'请填写姓名'" /></td>
	    		</tr>

	    		<tr>
	    			<td>性别:</td>
	    			<td colspan="4"><select class="easyui-combobox" data-options="editable: false, panelHeight: 50, width: 60, height: 30" name="gender"><option value="男">男</option><option value="女">女</option></select></td>
	    			
	    			<td>电话:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="phone" validType="mobile" /></td>
	    		</tr>

	    		<tr>
	    			<td>微信:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="wechat"  /></td>
	    			
	    			<td>QQ:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="qq" /></td>
	    		</tr>
	    		
	    		<tr>
	    			<td style="width:60px">照片:</td>
	    			<td colspan="4">
	    				<input  class="easyui-filebox" style="width: 200px; height: 30px;" name="photo" />
	    			</td>
	    			
	    			<td>生日:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-datebox" name="birthDay"  data-options="editable: false" /></td>
	    		</tr>

	    		<tr>
	    			<td>入职时间:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-datebox" name="entryDay"  data-options="editable: false"/></td>
	    			
	    			<td>基本工资:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-numberbox" name="basicSalary" precision="1" /></td>
	    		</tr>

	    		<tr>
	    			<td>主教课程:</td>
	    			<td colspan="4"><input id="majorCourseId" style="width: 200px; height: 30px;" class="easyui-combobox" type="text" name="majorCourseId" precision="1" data-options="editable: false"/></td>
	    			
	    			<td>课时费用:</td>
	    			<td colspan="4"><input style="width: 200px; height: 30px;" class="easyui-numberbox" type="text" name="classfees" precision="1" /></td>
	    		</tr>
	    	</table>
	    </form>
	</div>
